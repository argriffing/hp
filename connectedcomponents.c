    ...
    )
{
  ...
  assert(component_labels_out[root] == -1);
  component_labels_out[root] = label;
  ...
  // Pick the subgraph relevant to this component.
  ...
  subgraph->col_ind = p->compo_col_ind + p->nedges;

  // Update the maps between local and global vertex indices.
  subgraph->local_to_global[subgraph->nvertices] = root;
  p->global_to_local[root] = subgraph->nvertices;

  // Increment vertex counts to include the root vertex.
  p->nvertices++;
  subgraph->nvertices++;
